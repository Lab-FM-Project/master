...
unsigned char SaveFavourite(unsigned char buttonNo) {
    const unsigned int freqAddr[4] = {0xFFFF, 0x5B15, 0xD0B9};
    unsigned char address = freqAddr[buttonNo - 1];
    unsigned short freq;
    freq = frequency();
    eeprom_write(address, freq);
    PORTCbits.RC7 = 1;
    delay_10ms(10);
    PORTCbits.RC7 = 0;
    // Etc.
